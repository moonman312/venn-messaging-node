...
function MessagingClient(type) {
  ...
  var validateParams = function(type, data){
    if(type === "email"){
      ...
      if(data.to)if(!helpers.validatePhoneNumber(data.to)) return false;
      if(!helpers.validateMessage(data.message)) return false;
    }
    return true;
  }
  ...
  this.send = function(data, callback) {
    ...
    if(!validateParams(this.type, data)) return callback(new Error("Invalid parameters!"))
    vennApiService.getKeys( context.apiKey, context.type, function(err, keys) {
        ...
        context.sortServices(function(){
          ...
          context.sendRedundantly(data, function(err, result){
            ...
            vennApiService.postLog(context.apiKey, sendLog, function(err, result){
