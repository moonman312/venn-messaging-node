...
function MessagingClient(type) {
  ...
  // TODO: Validate per service basis for normalized error reponses
  var validateParams = function(type, data){
    if(type === "email"){
      if(!helpers.validateEmail(data.from)) return false;
      if(!helpers.validateEmail(data.to)) return false;
      if(!helpers.validateMessage(data.subject)) return false;
      if(!helpers.validateMessage(data.message)) return false;
    } else if(type === "push"){
      if(data.deviceType != "ios" && data.deviceType != "android") return false;
      if(!helpers.validateMessage(data.deviceToken)) return false;
      if(!helpers.validateMessage(data.message)) return false;
    } else if(type === "sms"){
      if(!helpers.validatePhoneNumber(data.to)) return false;
      if(data.from) if(!helpers.validatePhoneNumber(data.from)) return false;
      if(!helpers.validateMessage(data.message)) return false;
    }
    return true;
  }
  ...
  this.send = function(data, callback) {
    ...
    sendLog = []
    context.services = {}
    if(!context.apiKey) return callback(new Error("No valid API Key provided! Can't send!"))
    if(!validateParams(this.type, data)) return callback(new Error("Invalid parameters!")) //TODO: see validateParams
    vennApiService.getKeys( context.apiKey, context.type, function(err, keys) {
        ...
        if(helpers.isEmptyObject(context.services)) {
          return callback(new Error("No integrations turned on! Turn some on in your Venn Dashboard!"))
        }
