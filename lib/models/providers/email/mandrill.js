...
function Mandrill(keys){
  ...
  this.send = function(data, callback) {
    ...
    return this.client.messages.send({
      ...
      var mandrillStat;
      if(isSent){
        mandrillStat = MandrillServiceStatus(result[0], true);
...
function MandrillServiceStatus(response, success) {
  ...
  if (success) {
    ...
    // Queued parsing
    this.code = (response.status === 'queued' ? this.StatusCodes.QUEUED : this.StatusCodes.SUCCESS);

  } else {
    // There was an error when attempting to send message
    this.message = response.name + ': ' + response.message;

    if (response.name === 'PaymentRequired') this.code = this.StatusCodes.LIMIT_EXCEEDED;
    else if (response.name === 'GeneralError') this.code = this.StatusCodes.SERVICE_DOWN;
